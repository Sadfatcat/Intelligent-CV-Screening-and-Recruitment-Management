...
def _ranking_metrics(rows: list[dict[str, str]], k: int = 3) -> tuple[list[dict[str, str]], dict[str, float]]:
    ...
    for job_id, job_rows in sorted(by_job.items(), key=lambda item: item[0]):
        ranked = sorted(job_rows, key=lambda row: float(row["system_score"]), reverse=True)
        top_k = ranked[:k]
        relevant_total = sum(1 for row in job_rows if row["human_label"] in RELEVANT_LABELS)
        ...
        ideal_gains = sorted([int(row["human_relevance"]) for row in job_rows], reverse=True)[:k]
        ...
        metric_rows.append({
            ...
            "evaluated_pairs": str(len(job_rows)),
...
def main() -> None:
    labelled_path = EVALUATION_DIR / "evaluation_labelling_workbook.csv"
    results_path = EVALUATION_DIR / "evaluation_results.csv"
    summary_path = EVALUATION_DIR / "evaluation_summary.md"

    labelled_rows = _read_csv(labelled_path)
    ...
    for row in labelled_rows:
        ...
        if human_relevance != expected_relevance:
            ...
            continue
        system_score = _safe_float(row.get("existing_system_score", ""))
        if system_score is None:
            skipped.append(f"case_id={case_id}: missing system score")
            continue
    ...
    ranking_lines = [
        f"- job_id={row['job_id']} ({row['jd_name']}): "
    ...
    summary_path.write_text(
        "\n".join([
            "# Mini Quantitative Evaluation Summary",
            "",
            "This file is generated from `evaluation_labelling_workbook.csv` after manual labels are filled.",
            "",
            ...
            *([] if skipped else ["- None"]),
            ...
            "- Human labels are expected to be manually assigned or reviewed by the student.",
            "- This is a preliminary evaluation set, not a formal labelled benchmark.",
        ]),
        encoding="utf-8",
    )

    print(f"Wrote {results_path.relative_to(ROOT_DIR)} ({len(results)} evaluated rows)")
